...
require_once "/opt/gcs/php/config.php";
...
//  No front end on this system
	if ( $table['name'] == 'Panel' || $table['name'] == 'PanelGroup' || $table['name'] == 'PanelGroupPanel') {
		continue;
	} 	
...
// Deal with UUIDs
				if ($col['name'] == 'id') {
					if (!preg_match("/^[0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12}$/",$row[$col['name']])) {
						$row[$col['name']] = trim(`uuidgen`);
					}
				} 
				$myData = $row[$col['name']];
				$myCol = $col['name'];
